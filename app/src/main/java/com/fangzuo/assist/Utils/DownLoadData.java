...
public class DownLoadData {
    ...
    public  void alertToChoose() {
        ...
        CheckBox cb20 = v.findViewById(R.id.cb20);
        ...
        ab.setNeutralButton("下载全部", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                ...
                //源单类型16   往来科目17  价格政策18  入库类型19 公司信息表20
                for (int j = 1; j < 21; j++) {
                    if(j==5||j==8||j==11){
    ...
    private void downloadData(final ArrayList<Integer> choose) {
        ...
        App.getRService().downloadData(json, new MySubscribe<CommonResponse>() {
            ...
            public void onError(Throwable e) {
                pg.dismiss();
                SnackBarUtil.LongSnackbar(container, e.toString(), SnackBarUtil.Alert).setAction("重试", new View.OnClickListener() {
                    @Override
    ...
    private boolean insertLocalSQLite2(DownloadReturnBean dBean) {
        Lg.e("偷偷跑。。。asdfa。。");
        //后台存入物料数据，供退单查找使用
        BackDataService.getProducts(mContext);
        ...
        if (dBean.products != null && dBean.products.size() > 0) {
            ...
            productDao.insertInTx(dBean.products);
    ...
    private boolean insertLocalSQLite(DownloadReturnBean dBean) {
        ...
        if (dBean.companies != null && dBean.companies.size() > 0) {
            CompanyDao companyDao = session.getCompanyDao();
            companyDao.deleteAll();
            companyDao.insertOrReplaceInTx(dBean.companies);
            companyDao.detachAll();
        }
        return true;
    }
}
